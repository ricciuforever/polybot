...
def get_logger(name: str) -> logging.Logger:
    ...
    # 2. General Log (ROTATING - max 1MB, keeps 1 backup)
    fh = RotatingFileHandler("logs/bot.log", maxBytes=1*1024*1024, backupCount=1, encoding="utf-8")
    fh.setLevel(logging.INFO)
    fh.setFormatter(_file_fmt)
    logger.addHandler(fh)

    # 3. Error Log (ROTATING - solo ERROR e superiori, max 1MB)
    eh = RotatingFileHandler("logs/error.log", maxBytes=1*1024*1024, backupCount=1, encoding="utf-8")
    eh.setLevel(logging.ERROR)
    eh.setFormatter(_file_fmt)
    logger.addHandler(eh)

    # 4. Dashboard Log (Semplificato per la UI, max 1MB)
    dh = RotatingFileHandler("dashboard_log.txt", maxBytes=1*1024*1024, backupCount=1, encoding="utf-8")
